...
class GetPopularHomeTvShowsUseCase @Inject constructor(private val repository: TvShowRepository) {
    suspend operator fun invoke(): Flow<List<TvShow?>> {
        return repository.getPopularHomeTvShows().map { it.map { it.toHomeDomain() } }
    }
